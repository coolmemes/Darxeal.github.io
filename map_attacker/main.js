...
function loadUI() {
	...
	<table class='vis'>
    <tr>
        <th colspan='3'>Tribal Wars Map Attack Bot by Darxeal</th>
    </tr>
    <tr>
        <td>
            <table>
                <tr>
                    <td>Původ:</td>
                    <td>
                        <input type='text' id='source' style="width:42px;"> <span id="a_source">(klikni na mapě)</span>
                    </td>
                </tr>
                <tr>
                    <td>Cíl:</td>
                    <td><input type='text' id='target' style="width:42px;"> <span id="a_target">(klikni na mapě)</span>
                    </td>
                </tr>
                <tr>
                    <td><select size="1" id="o_send" style='width:40px'>
                            <option>Dopad:</option>
                            <option>Odchod:</option>
                        </select></td>
        ...
        <td>
            <table>`;
				...
				for (i = 0; i < units.length; i++) {
					html += "<th><img src='https://dscs.innogamescdn.com/8.86/33385/graphic/unit/unit_" + units[i] + ".png'></th>";
				}
				html += "<tr>";
				for (i = 0; i < units.length; i++) {
		...
		<td>
			<table>
				<tr>
					<td><img src='https://dscs.innogamescdn.com/8.86/33385/graphic/unit/unit_snob.png'></td>
					<td> <input type='number' style='width:40px' id='v_train' min="0"></td>
				</tr>
				<tr>
					<td><img src='https://dscs.innogamescdn.com/8.86/33385/graphic/unit/unit_catapult.png'></td>
	...
	attacks = [];
	map_legend.outerHTML = "<table id='attacks_table' class='vis'><tr><th colspan='3'>Povely</th><th>Jednotky</th><th>Trvání</th><th>Odchod</th><th>Příchod</th><th colspan='2'>Odchod za</th></tr></table>";
		...
		success: function (e) {
			...
			for (key in e[0].units) {
				if (attack.units[key]) {
					...
					} else if (traveltime > max_time) { max_time = traveltime; attack.slowest_unit = key; }
				}
			}
...
function updateVillage(coords, t, center = true) {
	...
	if (typeof TWMap.villages[coords] != "undefined") html = "<a href='javascript:updateVillage(" + coords + "," + t + ")'>" + TWMap.villages[coords].name.substring(0, 20) + "</a>"; else html = "N�zev nenalezen";
			...
			setTimeout(function (attack) {
				...
				setTimeout(function (attack) {
					if (typeof troop_confirm_go == "object") {
						...
						$("#a" + attack.id).html("<span style='color:green;'>Odesl�n</span>");
					} else {
...
if (window.location.href.indexOf("screen=map") != -1) loadUI(); else alert("Script pouštěj jen v mapě");
ctx = c.getContext("2d");
var waiting = false;
var adding = false;